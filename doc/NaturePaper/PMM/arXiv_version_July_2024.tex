...
\\
\\
\noindent {\bf \large Corresponding Author} ...
\end{comment}
...
\begin{equation}
\left[ \sum_{l=1}^p a_{j,l} c_l \right]^k = \sum_{k_1, k_2, \ldots, k_p} \binom{k_1+k_2+\cdots+k_p}{ k_1, k_2, \ldots, k_p} (a_{j,1}^{k_1} a_{j,2}^{k_2} \cdots a_{j,p}^{k_p})(c_1^{k_1} c_2^{k_2} \cdots c_p^{k_p}),
\end{equation}
and the functions
\begin{equation}\binom{k_1+k_2+\cdots+k_p}{k_1, k_2, \ldots, k_p} (a_{j,1}^{k_1} a_{j,2}^{k_2} \cdots a_{j,p}^{k_p})
\end{equation}
...
\blockcomment{
\clearpage
\section*{ALL SECTIONS BELOW ARE FOR REFERENCE ONLY}

\subsection*{Hamiltonian Emulation and Eigenstate Observables}
...
\begin{equation}
\begin{aligned}
    H(\{c_j\}) &= H_0 +\sum_jc_jH_j\\
      \hat{y}(\{c_j\}) &= [\hat{E}_k(\{c_j\}),\langle\psi_0(\{c_j\})|\hat{O}_k|\psi_0(\{c_j\})\rangle].
      \end{aligned}
\end{equation}

We form a PMM with same structure
\begin{equation}
    M(\{c_j\}) = \train{M_0} + \sum_j c_j\train{M_j},
\end{equation}
and calculate $k$ eigenstate energies and $k$ eigenstate observables in the PMM ground state
\begin{equation}
    y(\{c_j\}) = [E_k(\{c_j\}),\langle\phi_0(\{c_j\})|\train{O}_k|\phi_0(\{c_j\})\rangle],
\end{equation} 
where $\train{M_0},\train{M}_j,\train{O}\in\mathbf{C}^{\hyper{n}\times\hyper{n}}$ are Hermitian matrices.
Denote the trainable matrices, vectors, and scalars of the PMM by an underline. Likewise denote hyperparameters of the PMM by an overline.
The trainable parameters are trained by minimizing the mean squared error over all $N$ training points.
\begin{equation}
    \begin{aligned}
        \mathcal{L} = \frac{1}{N}\sum^N_{i=1}\left(\hat{y}_k(\{c^{(i)}_j\})-y_k(\{c^{(i)}_j\})\right)^2,
    \end{aligned}
\end{equation}
where $\hat{y}_k(\{c^{(i)}_j\})$ is the true data, and $y_k(\{c^{(i)}_j\})$ corresponds to the output of the PMM.

\subsection*{Zero Trotter Step Extrapolation}\label{sec:trotter_meth}
Given a $N\times N$ Hamiltonian that is a sum of $l$ local Hamiltonians 
\begin{equation}
\begin{aligned}
    H = \sum_{l=0}^mH_l.
\end{aligned}
\end{equation}
The time evolution operator for time duration $t$ corresponds to the exponential of the Hamiltonian, $\expm{-iHt}$. This is implemented as a product of evolution operators for small time steps $dt$,
\begin{equation}
\begin{aligned}
    U(dt) &= \expm{-iH_0dt}\expm{-iH_1dt}\cdots\expm{-iH_ldt}.
\end{aligned}
\end{equation}

Each term $U(dt) = \expm{-iHdt}$ can be approximated by the Trotter approximation as a product of exponentials of the individual terms comprising $H$, which is accompanied by an error $\mathcal{O}(dt^2)$.   

Once implemented on a quantum computer,  an algorithm is used to extract the energy levels using the complex phases produced during the time evolution of a quantum state. This data will then be used as snapshots of the energies  as a function of the time step $\hat{E}_k(dt)$.  

We apply a PMM with the same underlying structure,
\begin{equation}\label{eq:trotter_pmm_ham}
    M = \sum_{l=0}^m\train{M_l},
\end{equation}
and follow the same approximation.
\begin{equation}
\begin{aligned}
    U_M(dt) &= \expm{-i\train{M_0}dt} \expm{-i\train{M_1}dt}\cdots\expm{-i\train{M_l}dt},
\end{aligned}
\end{equation}
where each of the matrices, $\train{M_l}$, are $\hyper{n}\times \hyper{n}$ Hermitian matrices. The desired energies $E_k$ are determined by setting $\expm{-iE_kdt}$ equal to the eigenvalues of $U_M(dt)$
\begin{equation}
    \begin{aligned}
        E_k(dt) = \frac{ln(\Lambda[U_M(dt)]}{-idt}.
    \end{aligned}
\end{equation}
Here $\Lambda[\cdot]$ denotes the eigenvalues of $U_M(dt)$. The trainable parameters $\train{M_l}$ are trained by minimizing the mean squared error over all $J$ training points, namely
\begin{equation}
    \begin{aligned}
        \mathcal{L} = \frac{1}{J}\sum^J_i(\hat{E}_k(dt_i)-E_k(dt_i))^2,
    \end{aligned}
\end{equation}
where $\hat{E}_k(dt)$ is the true data, and $E_k(dt)$ corresponds to the output of the PMM. To obtain the $E(dt=0)$ result, one does a direct diagonalization of the trained PMM in Eq.~\eqref{eq:trotter_pmm_ham}

\subsection*{Anharmonic Lipkin-Meshkov-Glick PMM}

\subsection*{Regression PMMs}\label{sec:regression_meth}
Given dataset $X\in\R^\nxm{n}{m}$, we compute a ``Hamiltonian'' that is linear in the input features $x_i$
\begin{equation}
    \begin{aligned}
        H(X) = \train{H_0} +\sum_i^mx_i\train{H_i}.
    \end{aligned}
\end{equation}
We have defined $H_i\in\C^\nxm{\hyper{N}}{\hyper{N}}$. The eigensystem is then calculated for $H$
\begin{equation}
    ...
    \end{bmatrix}^\dagger.
\end{equation}
Ordering the eigenpairs by decreasing eigenvalue magnitude, $\abs{\lambda^{(k)}_1}\geq\abs{\lambda^{(k)}_2}\geq\cdots\geq\abs{\lambda^{(k)}_l}$, we select the first $\hyper{d}$ eigenvectors to ``decode'' into the output vector $z \in\R^c$,
\begin{equation}
    z_k = \train{g_k} + \sum_{i\leq j}^{\hyper{d}}\abs{v_i^\dagger \train{\Delta_{kij}} v_j}^2 - \frac{1}{2}\norm{\train{\Delta_{kij}}}_2^2,
\end{equation}
where $\train{\Delta_{kij}}\in\Herm\left(\hyper{l}\right)$ are trainable Hermitian ``decoder'' matrices and $\train{g}\in\R^c$ is a trainable bias vector. The trainable parameters are trained by minimizing a loss function over all $J$ training points
\begin{equation}
    \mathcal{L} = \sum_i^J\norm{\hat{z}_i-z_i}^P,
\end{equation}
where $\hat{z}$ is the true data, and $z$ is the output corresponding to the PMM.

%\subsection*{Hamiltonian Emulation\label{sec:anharmonic}}
...
%\begin{equation}
%\label{eq:DM}
%    H = B\sum_i r_i \sigma_i^z +J\sum_{i}(\sigma_i^x\sigma_{i+1}^x + \sigma_i^y\sigma_{i+1}^y+ \sigma_i^z\sigma_{i+1}^z) +  
%   D\sum_{i}(\sigma_i^x\sigma_{i+1}^y-\sigma_i^y\sigma_{i+1}^x),
%\end{equation}
%We included a longitudinal-field term to the model with random perturbations drawn from a uniform distribution ranging from $[0,1)$. For a $N=10$ qubit system, we used the values $B=4$,$J=8$, and $D=2.6$ given periodic boundary conditions. We choose 12 values of $dt$ for each energy level considered, where the starting $dt = ||H||_2^2/\pi$. A single $9\times9$ PMM was trained on all three energy levels following the procedure described in the Method section~\ref{sec:trotter_meth}. The point closest to $dt=0$ was used as a validation point for the PMM. We also trained a multi-layer perceptron (MLP) on all three energies as well. However, for its training process we employed grid search with leave-one-out cross-validation for hyperparameter tuning. Lastly we also compared to polynomial interpolation. 
%
%Since we do not have data at smaller values of $dt$, the polynomial fitting is less reliable than the Chebyshev polynomial interpolation used in Ref.~\cite{rendon2023improved}. The results are shown in Fig.~\ref{fig:DM}. The MLP and polynomial interpolation were not able to handle the sharp avoided level crossings involving the ground with the first excited state and the first with second excited states. In contrast, the PMM reproduces the avoided level crossings properly---which are not present in the training data---allowing the PMM to extrapolate through interpolation to $dt = 0$ with much higher accuracy. 
%

For the list of 2D functions the training data was obtained from sampling 200 points in the space randomly given a uniform distribution from $[0,1)$. The test data we sampled 10000 points on a grid from $[0,1]$. 
We used the same $7x7$ PMM with $d=3$ for all functions given the procedure outlined in the Methods, Section~\ref{sec:regression_meth}. 10\% of the training set for the PMM was used as a validation set. We then compared the results to a multitude of machine learning (ML) algorithms, where each ML algorithm was trained using grid search with cross-validation for hyperparameter tuning for each function.

For the polynomial and Fourier series dataset the training data was obtained from sampling 200 points in the space randomly given a uniform distribution from $[-1,1)$. The test data we sampled 10000 points on a grid from $[-1,1]$.
We used the same $9x9$ PMM with $d=3$ for all 10000 polynomials and 10000 Fourier series. 10\% of the training set for the PMM was used as a validation set. We then compared the results to a multitude of machine learning (ML) algorithms, where each ML algorithm was trained using grid search with cross-validation for hyperparameter tuning for every single polynomial and Fourier series in the 10000 functions included in the dataset.

For the NASA/CERN datasets 35\% of the dataset was used as the train set. The ML models used the whole train set with grid search cross-validation. For PMM 10\% of the train set was used a validation set. For NASA a $7\times7$ with $d=3$ PMM was used. for the CERN a $15\times15$ with $d=3$ PMM was used. 
...
% $\ddagger$ $(x,y)\in[-1,1]\times[-1,1]$
% \end{tblr}
% \caption{\label{tab:math_functions}Collection of mathematical functions used for testing regression performance.}
% \end{table}

\subsection*{Image Classification PMMs}
(This is an extremely rough draft, just to get the maths written out.)

Given an image $X\in\R^\nxm{n}{m}$, we select $w$ windows of shape $\nxm{r_i}{s_i}$. For each window $X_i$ the products $X_iX_i^T\in\R^\nxn{r}$ and $X_i^T X_i\in\R^\nxn{s}$ are computed as surrogates for the row-wise and column-wise correlation matrices, respectively. For each surrogate correlation matrix, the entries are uniformly normalized element-wise to lie in the range $\left[0,1\right]$, and finally the diagonal elements are set to unity. Denote these post-normalized products by $R_i$ and $C_i$,
\begin{equation}
    R_i \stackrel{\sim}{\propto} X_i X_i^T,\quad C_i \stackrel{\sim}{\propto} X_i^T X_i.
\end{equation}
Note that this step can be a preprocessing step, or included in the inference process of the PMM. This allows the PMM to operate efficiently on the training set as well as adapt to new previously-unseen data.

Denote the trainable matrices, vectors, and scalars of the PMM by an underline $\train{M}$, $\train{v}$, $\train{a}$. Likewise denote hyperparameters of the PMM by an overline, such as the number and shape of windows $\hyper{w}$, $\hyper{r_i}$, $\hyper{s_i}$. The inference process of the PMM which aims to predict $c$ class probabilities for an image, $X$, begins with encoding the image into the row- and column-wise correlation matrix surrogates for various windows as mentioned above. Then
\begin{equation}
    M = \sum_i^{\hyper{w}} \train{K_i}R_i\train{K_i}^\dagger + \train{L_i}C_i\train{L_i}^\dagger,
\end{equation}
encodes these surrogate correlation matrices across all windows into a single Hermitian matrix $M\in\Herm\left(\hyper{k}\right)$ using trainable ``compressor'' matrices $\train{K_i}\in\C^\nxm{\hyper{k}}{\hyper{r_i}}$ and $\train{L_i}\in\C^\nxm{\hyper{k}}{\hyper{s_i}}$. Next we compute a ``Hamiltonian'' for each possible output class $k$,
\begin{equation}
    H_k = \train{D_k}M\train{D_k}^\dagger + \train{B_k},\quad k = 1, 2, \ldots, c,
\end{equation}
where $\train{D_k}\in\C^\nxm{\hyper{l}}{\hyper{k}}$ are trainable ``discriminator'' matrices and $\train{B_k}\in\Herm\left(\hyper{l}\right)$ are trainable bias Hermitian matrices. The eigensystem is then calculated for each $H_k$
\begin{equation}
    H_k = V_k\Lambda_k V_k^\dagger = %
    \begin{bmatrix}
        \vert & \vert & \cdots & \vert \\
        v_1^{(k)} & v_2^{(k)} & \cdots & v_{\hyper{l}}^{(k)}\\
        \vert & \vert & \cdots & \vert
    \end{bmatrix}%
    \begin{bmatrix}
        \lambda^{(k)}_1 & & & \\
    ...
    \end{bmatrix}^\dagger.
\end{equation}
Ordering the eigenpairs by decreasing eigenvalue magnitude, $\abs{\lambda^{(k)}_1}\geq\abs{\lambda^{(k)}_2}\geq\cdots\geq\abs{\lambda^{(k)}_l}$, we select the first $\hyper{d}$ eigenvectors to ``decode'' into the penultimate output vector $z\in\R^c$,
\begin{equation}
    z_k = \train{g_k} + \sum_{i\leq j}^{\hyper{d}}\abs{v_i^{(k)\dagger} \train{\Delta_{kij}} v_j^{(k)}}^2 - \frac{1}{2}\norm{\train{\Delta_{kij}}}_2^2,
\end{equation}
where $\train{\Delta_{kij}}\in\Herm\left(\hyper{l}\right)$ are trainable Hermitian ``decoder'' matrices and $\train{g}\in\R^c$ is a trainable bias vector. Note that this step is constructed especially to preserve unitary invariance as well as unbounded output range. Finally, to convert this vector to class probabilities, a standard $\softmax$ with temperature $\hyper{\tau}$ is applied,
\begin{equation}
\quad p_k = \softmax\left(z_k/\hyper{\tau}\right) = \frac{\exp(z_k/\hyper{\tau})}{\sum_{k'=1}^c \exp(z_{k'}/\hyper{\tau})}.
\end{equation}
The trainable parameters are trained by minimizing the mean cross-entropy loss over all $N$ training examples,
\begin{equation}
    \mathcal{L} = -\frac{1}{N}\sum\limits_n^N \sum\limits_k^c \hat{p}^{(n)}_k \ln p^{(n)}_k
\end{equation}
where $\hat{p}^{(n)}_k$ is the true probability for sample $n$ to lie in class $k$, and $p^{(n)}_k$ is the corresponding predicted probability from the PMM.
