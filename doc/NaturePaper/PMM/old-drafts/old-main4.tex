...
%\linenumbers
\title{Matrix Model Emulator and Encoder}
...
\twocolumn[
  \begin{@twocolumnfalse}
    \maketitle
    \begin{abstract}
        This paper presents a novel deep learning architecture called Matrix Model Emulator (MME) and a new method to encode and extract the spectral information of an image. Stemming from the MME concept, we crafted two separate  data-driven emulators to show its capabilities and address specific challenges within physics. Emulators have the potential of bypassing the computational burdens of complex scientific calculations without compromising accuracy. The  Eigenvalue Emulator, a dimensionality reduction method, designed to leverage the structure of Eigenvector Continuation (EC), enables fast calculation of extremal eigenvalues, other observables, and identification of branch points without the explicit formation of norm and Hamiltonian matrices.  It's performance is validated against EC and other interpolation techniques on the Anharmonic Oscillator. The second method is the Trotter Emulator which serves as a post-processing technique aimed at diminishing Trotter errors in Quantum Computers, with its efficacy demonstrated on the Heisenberg spin chain model. Lastly, from the structure of MME, we developed an image encoder that extracts data from an image that was not previously accessible. Our findings highlight that MMEs as data-driven machine learning model excel or match their counterparts. The results offer valuable insights for further development and potential applications of MMEs in nuclear physics and beyond.
    \end{abstract}
  \end{@twocolumnfalse}]
...
\par
Thus we have developed a novel deep learning architecture called Matrix Model Emulator (MME). Stemming from the MME concept, we crafted two separate data-driven emulators to address specific challenges within physics, and an image encoder. 
\par
...
\par
This paper is organized into a series of sections. The second section introduces the Matrix Model Emulator (MME), with a description of the two Emulators and image encoder that we have designed. In the third section, we present our results for all three methods. Finally, the fourth section encapsulates our thoughts from the study. 

%########################################
\section{Matrix Model Emulator and Encoder}
...
%########################################
\subsection{Encoder: Unsupervised Learning}
Over the past decade, significant advancements have taken place in the realm of image classification, driven by both supervised and unsupervised learning techniques. These methods encompass a wide spectrum, ranging from traditional approaches like Kernel PCA (KPCA), K-Means, and t-SNE to cutting-edge deep learning methods, notably Convolutional Neural Networks (CNNs). It is well-established that the performance of most machine learning models benefits from a wealth of data, with larger datasets often yielding better results.

To harness the potential of machine learning models with a greater volume of data, we introduce the Matrix Model Encoder (MME). The MME is designed to encode images into a potential for a matrix  $M(c)$ as depicted in Eq~\ref{MM_encoder}. This encoding process enables the extraction of previously inaccessible information from the images, enriching the data available for machine learning models (including MME~\ref{MME_unsup}.) Subsequently, on top of the original image dataset, machine learning models are supplied with the energy spectrum  representation as input.
\begin{figure}
    \centering
    \includegraphics[width=0.4\textwidth]{Figures/kpca.png}
    \caption{Kernel PCA applied to the MNIST dataset. The left graph displays the results when KPCA is applied solely to the original dataset, projecting it into two principal components. On the right, KPCA is employed on both the original dataset and the encoded energy spectrum obtained from MME, resulting in a projection into two principal components.}
    \label{fig:kpca}
\end{figure}
\par
The availability of the supplementary energy spectrum dataset serves to augment the capabilities of other machine learning models, as illustrated in Figure~\ref{fig:kpca}. This figure vividly demonstrates the impact on KPCA's performance when applied to the MNIST dataset. When combined with the energy spectrum provided by MME, KPCA exhibits improved performance compared to its performance without this additional information, especially when projecting the data into two principal components.
\par
The results serve as compelling evidence for the power of this novel encoding scheme, which leverages the architecture of MME for image recognition tasks.
%#####################################
\section{Conclusion}
...
the underlying dimension is the circle that each point belongs to. In Fig.~\ref{fig:MPCA} we show the unsupervised MME approach applied
